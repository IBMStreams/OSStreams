 ...
 * eviction and CountWindowPolicy for trigger.
 */
class WindowTest7 : public WindowTestBase
{
  public:
  ...
  private:
    void runTests()
    {
        ...
        CountWindowPolicy policy_C5(trigSize);

        {
            // Sliding windows without partition eviction
            test_winSlidingDeltaCount_resetInitial(oper, policy_Dage2, policy_C5);
            test_winSlidingDeltaCount_reset(oper, policy_Dage2, policy_C5, 3);
            test_winSlidingDeltaCount_reset(oper, policy_Dage2, policy_C5, 4);
            test_winSlidingDeltaCount_reset(oper, policy_Dage2, policy_C5, 5);
            test_winSlidingDeltaCount_reset(oper, policy_Dage2, policy_C5, 10);
        }
    }

    void test_winSlidingDeltaCount_resetInitial(Operator& oper,
                                                ...
                                                WindowPolicy const& trigger)
    {
...
#endif
        SlidingDeltaCountHandler handler(this->winSize, this->trigSize);
        ...
        ASSERT_EQUALS(z, handler.numBeforeInsertion_[partitionZero]);
        ASSERT_EQUALS(z, handler.numAfterInsertion_[partitionZero]);
        ASSERT_EQUALS(2 + this->trigSize, handler.numBeforeEviction_[partitionZero]);
        ASSERT_EQUALS(2 + this->trigSize, handler.numAfterEviction_[partitionZero]);
        ...
        ASSERT_EQUALS(z - 1, handler.numBeforeInsertion_[partitionZero]);
        ASSERT_EQUALS(z - 1, handler.numAfterInsertion_[partitionZero]);
        ASSERT_EQUALS(4 + this->trigSize, handler.numBeforeEviction_[partitionZero]);
        ASSERT_EQUALS(4 + this->trigSize, handler.numAfterEviction_[partitionZero]);
        ASSERT_EQUALS(2, handler.numOnTrigger_[partitionZero]);
        ASSERT_EQUALS(1, handler.numOnInitialFull_[partitionZero]);
     ...
     */
    void test_winSlidingDeltaCount_reset(Operator& oper,
                                         WindowPolicy const& evict,
                                         WindowPolicy const& trigger,
                                         const int checkpointCount)
    {
        ...
        // Checkpoint if checkpointCount == winSize+3
        checkpointAtCount(win, ckptSave, z, checkpointCount);
        ...
        // Checkpoint if checkpointCount == winSize + 3 + trigSize
        checkpointAtCount(win, ckptSave, z, checkpointCount);
        ...
        int totalInsertedTuples = z - 1;
        for (z = checkpointCount; z < totalInsertedTuples - 1; ++z) {
            tuple.get_age() = z;
            win.insert(tuple);
