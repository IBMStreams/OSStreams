...
sub verify($)
{
    ...
    my $close = $closeMode ? $closeMode->getValueAt(0)->getSPLExpression() : 'never';
    $retryFailedSends =
        $retryFailedSends ? $retryFailedSends->getValueAt(0)->getSPLExpression() : 0;
    ...
    # dynamic closeMode has many restrictions
    if ($close eq 'dynamic') {
        SPL::CodeGen::exitln(SPL::Msg::STDTK_TCPSINK_DYNAMIC_NEED_RETRY_FAILED_SENDS(),
                           $closeMode->getSourceLocation())
            if !$retryFailedSends;
        SPL::CodeGen::exitln(SPL::Msg::STDTK_TCPSINK_DYNAMIC_SERVER(),
                           $closeMode->getSourceLocation())
            if $role ne 'client';
        SPL::CodeGen::exitln(SPL::Msg::STDTK_TCPSINK_DYNAMIC_NEED_PARAM('address'),
                           $closeMode->getSourceLocation())
            if !$address;
    } else {
        SPL::CodeGen::exitln(SPL::Msg::STDTK_TCPSINK_USES_ATTRIBUTES('address'),
                           $address->getSourceLocation())
            if $address && $address->getValueAt(0)->hasStreamAttributes();

        SPL::CodeGen::exitln(SPL::Msg::STDTK_TCPSINK_USES_ATTRIBUTES('port'),
                           $port->getSourceLocation())
            if $port && $port->getValueAt(0)->hasStreamAttributes();
    }
