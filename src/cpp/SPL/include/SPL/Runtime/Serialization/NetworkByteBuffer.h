...
#ifndef SPL_RUNTIME_SERIALIZATION_NETWORK_BYTE_BUFFER_H
#define SPL_RUNTIME_SERIALIZATION_NETWORK_BYTE_BUFFER_H

/*!
 * \file NetworkByteBuffer.h \brief Definition of the SPL::NetworkByteBuffer class.
 */
...
namespace SPL
{
    ...
    /// managed (allocated/resized/deallocated) by the NetworkByteBuffer
    ...
    /// underlying memory will have to be deallocated externally, unless \c
    ...
    class DLL_PUBLIC NetworkByteBuffer : public ByteBuffer<NetworkByteBuffer>
    {
    public:
        /// Default Constructor
        ///
        ...
        /// NetworkByteBuffer (the default behavior is for the memory management
        ...
        /// Copy constructor
        /// @param sb an existing NetworkByteBuffer object
        ...
        {
            uint16_t temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<uint16_t>(temp).data;
        ...
        int16_t getInt16()
        {
            int16_t temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<int16_t>(temp).data;
        ...
        uint32_t getUInt32()
        {
            uint32_t temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<uint32_t>(temp).data;
        ...
        int32_t getInt32()
        {
            int32_t temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<int32_t>(temp).data;
        ...
        uint64_t getUInt64()
        {
            uint64_t temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<uint64_t>(temp).data;
        ...
        int64_t getInt64()
        {
            int64_t temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<int64_t>(temp).data;
        ...
        float getFloat()
        {
            float temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<float>(temp).data;
        ...
        double getDouble()
        {
            double temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<double>(temp).data;
        ...
        void addLongDouble(long double d)
        {
            ...
            ensure(sizeof (Streams::HostNetworkTmp<long double>));
            memcpy(buf+icursor,Streams::host_to_network<long double>(d).rawData,
                   sizeof (Streams::HostNetworkTmp<long double>));
            icursor += sizeof (Streams::HostNetworkTmp<long double>);
        }
        ...
        long double getLongDouble()
        {
            long double temp;
            memcpy (&temp, buf+ocursor, sizeof temp);
            temp = Streams::network_to_host<long double>(temp).data;
            ocursor += sizeof (Streams::HostNetworkTmp<long double>);
            return temp;
        }
        ...
        /// @throws a SPLRuntimeDeserializationException if a pointer cannot be
        ...
        void * getPointer()
        {
            ...
            assert(sizeof(void *)==sizeof(int64_t));
            if (c != PTR_64BIT) {
                THROW_STRING(SPLRuntimeDeserialization, SPL_APPLICATION_RUNTIME_NETBB_POINTER(64));
            }
            ...
            assert(sizeof(void *)==sizeof(int32_t));
            if(c!=PTR_32BIT)
                THROW_STRING(SPLRuntimeDeserialization, SPL_APPLICATION_RUNTIME_NETBB_POINTER(32));
...
#include <SPL/Runtime/Serialization/NetworkByteBufferHelpers.h>

#endif /* SPL_RUNTIME_SERIALIZATION_NETWORK_BYTE_BUFFER_H */
