...
// Class Description:
//
// Implements a resizable buffer used for serialization purposes -- this
// version does no byte flipping (NBF = No Byte Flipping)
...
/// by the SBufferNBF class. Such a behavior can be changed by handing a
...
/// to the class
class SBufferNBF : public SerializationBuffer
{
  public:
    /// Default Constructor
    SBufferNBF(void);
    ...
    SBufferNBF(const unsigned initialSize, const bool myCxxMemAllocStyle = true);
    /// Constructor -- used for handing externally allocated buffers to
    /// SBufferNBF (the default behavior is for the memory management
    ...
    SBufferNBF(unsigned char* mybuf, const unsigned mysize);
    ...
    /// @param mysize the buffer size (this buffer is allocated externally --
    /// resizings can only be done later if autoDealloc is set to be true)
    SBufferNBF(char* mybuf, const unsigned mysize);

    /// Copy constructor
    /// @param sb an existing SBufferNBF object
    SBufferNBF(const SBufferNBF& sb);
    ...
    inline void addUChar(const UChar ch)
    {
        ...
        memcpy(buf + icursor, &ch, sizeof(UChar));
    ...
    inline void addUChar(const unsigned offset, const UChar ch)
    {
        ...
        memcpy(buf + offset, &ch, sizeof(UChar));
    }
    ...
    inline UChar getUChar(void)
    {
        ...
        memcpy(reinterpret_cast<char*>(&i), buf + ocursor, sizeof(UChar));
    ...
    inline UChar getUChar(unsigned offset) const
    {
        ...
        memcpy(reinterpret_cast<char*>(&i), buf + offset, sizeof(UChar));
        return i;
    }
    ...
    inline void addUInt16(const unsigned offset, const uint16_t i)
    {
        ...
        memcpy(buf + offset, &i, sizeof(uint16_t));
    }
    ...
    inline uint16_t getUInt16(const unsigned offset) const
    {
        ...
        memcpy(&i, buf + offset, sizeof(uint16_t));
        return i;
    }
    ...
    inline int16_t getInt16(const unsigned offset) const
    {
        ...
        memcpy(&i, buf + offset, sizeof(int16_t));
        return i;
    }
    ...
    inline uint32_t getUInt32(const unsigned offset) const
    {
        ...
        memcpy(&i, buf + offset, sizeof(uint32_t));
        return i;
    }
    ...
    inline void addUInt64(const unsigned offset, const uint64_t i)
    {
        ...
        memcpy(buf + offset, &i, sizeof(uint64_t));
    }
    ...
    inline uint64_t getUInt64(const unsigned offset) const
    {
        ...
        memcpy(&i, buf + offset, sizeof(uint64_t));
        return i;
    }
    ...
    void addInt64(const unsigned offset, const int64_t i)
    {
        ...
        memcpy(buf + offset, &i, sizeof(int64_t));
    }
    ...
    {
        unsigned stlen = strlen(reinterpret_cast<char*>(buf) + ocursor) + 1;
    ...
    {
        unsigned stlen = strlen(reinterpret_cast<char*>(buf) + offset) + 1;
        if (offset + stlen > size) {
            THROW(Serialization, "attempt to read beyond buffer limits");
        }
    ...
    virtual bool operator==(const SBufferNBF& b) const;

    /// Destructor
    ///
    virtual ~SBufferNBF(void);
...
#endif
        if (!autoDealloc) {
            THROW(Serialization, "attempt to resize external buffer");
        }
