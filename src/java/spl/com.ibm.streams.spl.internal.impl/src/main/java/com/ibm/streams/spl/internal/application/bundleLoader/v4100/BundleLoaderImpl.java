...
package com.ibm.streams.spl.internal.application.bundleLoader.v4100;
...
public class BundleLoaderImpl implements BundleLoader {
  ...
  private Options constructOptions() {
    ...
    Option helpOpt = OptionBuilder.withLongOpt(helpOptionName).create("h");
    options.addOption(helpOpt);

    Option logStdErrOpt = OptionBuilder.withLongOpt(logToStdErr).create("c");
    options.addOption(logStdErrOpt);

    Option exeDirOpt = OptionBuilder.withLongOpt(exeDirOptionName).hasArg().create("e");
    options.addOption(exeDirOpt);

    Option killAfterOpt = OptionBuilder.withLongOpt(killAfter).hasArg().create("k");
    options.addOption(killAfterOpt);

    Option logLevelOpt = OptionBuilder.withLongOpt(logLevel).hasArg().create("l");
    options.addOption(logLevelOpt);

    Option traceLevelOpt = OptionBuilder.withLongOpt(traceLevel).hasArg().create("t");
    options.addOption(traceLevelOpt);

    return options;
  }

  private String computeExecutionDirectoryRoot(ArrayList<String> args) throws Exception {
    ...
    try {
      ...
      if (commandLine.hasOption(exeDirOptionName)) {
        String value = commandLine.getOptionValue(exeDirOptionName);
        // value is the location we need to use as a temporary root directory
        File root = new File(value).getAbsoluteFile();
        if (!root.exists()) {
          throw new Exception("The execution location " + value + " does not exist.");
        } else if (!root.isDirectory()) {
          throw new Exception("The execution location " + value + " is not a directory.");
        } else if (!(root.canExecute() && root.canRead() && root.canWrite())) {
          throw new Exception(
              "The execution location " + value + " does not have the correct permissions.");
        }
        return root.toString();
      }

    } catch (final ParseException e) {
      throw new Exception(e.getMessage() + " is not a recognized option");
    }
    return new String(File.separator + "tmp");
  }

  @Override
  public int runStandalone(ArrayList<String> args) throws IOException {

    // Look for -e or --execution-directory in the options
    String executionDirRoot;
    try {
      executionDirRoot = computeExecutionDirectoryRoot(args);
    } catch (Exception e) {
