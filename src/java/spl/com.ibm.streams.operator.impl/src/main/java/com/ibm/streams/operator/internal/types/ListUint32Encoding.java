...
class ListUint32Encoding implements TypeEncoding {
  ...
  final List<Integer> readList(ByteBuffer bytes) {
    final int n = getBLen().getLength(bytes);
    if (n == 0) return Collections.emptyList();

    int[] data = new int[n];
    for (int i = 0; i < n; i++) data[i] = bytes.getInt();
  ...
  public long getMaxSerializedSize(Object value) {
    ...
    return getBLen().getSerializedSize() + (4 * list.size());
  }
  ...
  final int writeList(ByteBuffer bytes, Object value) {
    ...
    if (list instanceof IntegerList) {
      ((IntegerList) list).putInternal(bytes);
    } else {
      for (int i = 0; i < n; i++) bytes.putInt((Integer) list.get(i));
    }
    return n;
  }
}
