...
/** Evict partitions according to the total number of partitions in the window. */
public class PartitionCountEvictionHandler<T> extends OrderedPartitionHandler<T> {

  PartitionCountEvictionHandler(
  ...
  void insertIntoPartition(T tuple) throws Exception {
    ...
    if (needsPartitionEviction()) countBasedEviction();
  }

  boolean needsPartitionEviction() {

    synchronized (orderedPartitions) {
      return orderedPartitions.size() > evictionConfiguration;
    }
  }
}
