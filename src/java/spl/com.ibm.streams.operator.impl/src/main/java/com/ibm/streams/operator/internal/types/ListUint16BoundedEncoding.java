...
class ListUint16BoundedEncoding extends ListUint16Encoding {

  private final BoundInfo bound;
  private final int fixedLength;

  ListUint16BoundedEncoding(BoundInfo bound) {
    super(bound.getSizeType());
    this.bound = bound;
    fixedLength = getBLen().getSerializedSize() + (2 * bound.getBoundedLength());
  }

  @Override
  public Object readValue(ByteBuffer bytes) {
    List<Short> list = readList(bytes);
    int unused = bound.getBoundedLength() - list.size();
    if (unused != 0) TupleSerialization.skip(bytes, unused, 2);
    return list;
  }
