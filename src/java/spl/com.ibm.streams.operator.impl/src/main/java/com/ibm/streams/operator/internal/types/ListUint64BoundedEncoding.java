...
class ListUint64BoundedEncoding extends ListUint64Encoding {

  private final BoundInfo bound;
  private final int fixedLength;

  ListUint64BoundedEncoding(BoundInfo bound) {
    super(bound.getSizeType());
    this.bound = bound;
    fixedLength = getBLen().getSerializedSize() + (8 * bound.getBoundedLength());
  }

  @Override
  public Object readValue(ByteBuffer bytes) {
    List<Long> list = readList(bytes);
    int unused = bound.getBoundedLength() - list.size();
    if (unused != 0) TupleSerialization.skip(bytes, unused, 8);
    return list;
  }
