...
import com.ibm.streams.platform.services.v4200.AttrType;
import com.ibm.streams.platform.services.v4200.ListOrSetType;
import com.ibm.streams.platform.services.v4200.MapType;
import com.ibm.streams.platform.services.v4200.OptionalType;
import com.ibm.streams.platform.services.v4200.TupleType;
import com.ibm.streams.platform.services.v4200.TypeType;
...
public class TopologyTypeFactory {
  ...
  private static TopologyType getTopologyType(AttrType adlType) {
    ...
    if (adlType.getSet() != null) {
      return getTopologySetType(adlType.getSet());
    }
    assert (adlType.getOptional() != null);
    return getTopologyType(adlType.getOptional());
  }

  private static TopologyType getTopologyType(OptionalType adlType) {
    TopologyType underlyingType = null;
    if (adlType.getType() != null) {
      underlyingType = getTopologyType(adlType.getType());
    } else if (adlType.getTuple() != null) {
      underlyingType = getTopologyType(adlType.getTuple());
    } else if (adlType.getList() != null) {
      underlyingType = getTopologyListType(adlType.getList());
    } else if (adlType.getMap() != null) {
      underlyingType = getTopologyType(adlType.getMap());
    } else {
      assert (adlType.getSet() != null);
      underlyingType = getTopologySetType(adlType.getSet());
    }
    return new TopologyOptionalType(underlyingType);
  }
  ...
  private static TopologyType getTopologyListType(ListOrSetType adlType) {
    ...
    } else if (adlType.getOptional() != null) {
      elementType = getTopologyType(adlType.getOptional());
    } else {
      assert (adlType.getSet() != null);
