...
public class AnnotationFactory {
  ...
  public static Annotation createAnnotation(
      String tag, List<SplAnnotationValueType> splValues, SPLLogicalModel model) {
    ...
    if (tag.compareTo("view") == 0) {
      ...
      for (SplAnnotationValueType annotValue : splValues) {
        ...
        if (key.compareTo("filter") == 0) {
          ...
          for (String s : keyVals) {
            ...
            // Note that we are not supporting submission-time values here
            filterKeyValues.add(
          ...
          for (String s : attributes) {
            ...
            // Note that we are not supporting submission-time value here
      ...
      if (tag.compareTo("logicClause") == 0 || tag.compareTo("consistentRegion") == 0) {
        isInternal = true;
      }
      for (SplAnnotationValueType annotValue : splValues) {
        String key = annotValue.getKey();
