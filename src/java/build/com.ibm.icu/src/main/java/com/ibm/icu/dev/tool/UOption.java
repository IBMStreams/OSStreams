...
public class UOption {
   ...
   * form of an array of UOption structures. Each UOption defines a long and a short name (a string
   * and a character) for options like "--foo" and "-f".
   *
   * <p>Each option is marked with whether it does not take an argument, requires one, or optionally
   * takes one. The argument may follow in the same argv[] entry for short options, or it may always
   * follow in the next argv[] entry.
   *
   ...
   * unless it is only the '-' itself. There is no restriction of the argument format if it is part
   * of the short name options's argv[] entry.
   *
   ...
   * @param start the first argument in argv[] to examine. Must be 0..argv.length-1. Arguments from
   *     0..start-1 are ignored.
        ...
        if (c == '-') {
          ...
          if (arg.length() == 0) {
            ...
            if (option.hasArg != NO_ARG) {
              ...
              if (i + 1 < argv.length
                  && !(argv[i + 1].length() > 1 && argv[i + 1].charAt(0) == '-')) {
